...
router.post('/', authenticateJWT, async (req: Request, res: Response) => {
    ...
    try {
        const result = await query(
            'INSERT INTO reviews (author, position, content) VALUES ($1, $2, $3) RETURNING *',
...
router.put('/:id', authenticateJWT, async (req: Request, res: Response) => {
    ...
    try {
        const result = await query(
            'UPDATE reviews SET author = $1, position = $2, content = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $4 RETURNING *',
            [author, position, content, id]
        );
